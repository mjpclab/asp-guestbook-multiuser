...
<div id="outerborder" class="outerborder">
	...
	<table border="1" bordercolor="<%=TableBorderColor%>" cellpadding="2" class="generalwindow">
		<tr>
			<td class="centertitle">内容过滤策略（优先于普通用户设置）</td>
		</tr>
		<tr>
			<td class="wordscontent" style="padding:20px 2px;">
				<form method="post" name="newfilter" action="web_appendfilter.asp" onsubmit="if(findexp.value==''){alert('请输入查找内容。');findexp.focus();return false;}submit1.disabled=true;">
				...
				while rs.EOF=false%>
					...
					<form method="post" action="web_updatefilter.asp">
						...
						<input type="button" value="删除" onclick="if (confirm('确实要删除该条策略吗？')) {this.form.action='web_delfilter.asp';this.form.submit();}" /> 
						<input type="button" value="上移" onclick="{this.form.movedirection.value='up';this.form.action='web_movefilter.asp';this.form.submit();}" /> 
						<input type="button" value="下移" onclick="{this.form.movedirection.value='down';this.form.action='web_movefilter.asp';this.form.submit();}" /> 
						<input type="button" value="移至顶层" onclick="{this.form.movedirection.value='top';this.form.action='web_movefilter.asp';this.form.submit();}" /> 
						<input type="button" value="移至底层" onclick="{this.form.movedirection.value='bottom';this.form.action='web_movefilter.asp';this.form.submit();}" /> 
