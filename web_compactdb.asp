...
if err.number<>0 then
	Call TipsPage("服务器不支持FSO(File System Object)，操作无法继续。","web_admin.asp")
	response.end
end if
on error goto 0
end sub
...
elseif dbtype>=10 and dbtype<=13 then
	...
	Call CreateConn(cn)
	cn.Execute sql_compact_dbfile,,129
	cn.Execute sql_compact_dblog,,129

	cn.Close
	set cn=nothing
end if

Call TipsPage("数据库压缩完成。","web_admin.asp")
%>