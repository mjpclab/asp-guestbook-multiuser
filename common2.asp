			...
			<div class="embedbox">
				<table class="embedbox" cellpadding="0" cellspacing="0"><tr><td>
					...
					if encrypted=true and pagename<>"showword" and left(pagename,5)<>"admin" then
						response.Write "<img alt="""" src=""image/icon_key.gif"" class=""imgicon"" />(需要预设的密码才能查看...)[<a href=""showword.asp?user=" &rs("adminname")& "&id=" &rs2("id")& """>点击这里验证...</a>]"
					else
						...
						<%=t_rs.Fields("name") & "： (" & t_rs.Fields("logdate") & ") "%>
	  					...
	  					if (iswhisper=false and clng(guestflag and 256)=0) or (pagename="showword" and needverify) or left(pagename,5)="admin" then
		  					...
		  					if left(pagename,5)="admin" then
								...
								if AdminShowOriginalIP>0 and t_rs("originalip")<>"" then Response.Write "<img alt="""" src=""image/icon_ip2.gif"" alt=""原始IP：" &getip(t_rs("originalip"),cint(AdminShowOriginalIP))& """ class=""ipicon""" &js_status& " /> "
		  					else
...
sub outerword(byref t_rs)
	...
	<tr>
		...
		<td class="tableright"<%if ReplyInWord then response.write " style=""padding-bottom:12px;"""%>>
			<table cellpadding="2" cellspacing="0" style="width:100%; border-width:0px; border-collapse:collapse; margin-left:auto; margin-right:auto;">
	  			<tr>
	  			...
	  			<tr>
	  				<td class="wordscontent">
	  					<%
						if t_rs.Fields("parent_id")<=0 and left(pagename,5)<>"admin" and left(pagename,3)<>"web" then showguestreplyicons t_rs.Fields("id"),t_rs.Fields("parent_id"),StatusWrite and StatusGuestReply and clng(t_rs.Fields("guestflag") and 512)=0
