	...
	<table border="1" bordercolor="<%=TableBorderColor%>" cellpadding="2" class="generalwindow">
		...
		<tr>
			...
			<tr style="cursor:pointer;">
				<td style="width:20%;" onclick="window.location='admin_config.asp?user=<%=ruser%>&amp;page=1'" onmouseover="this.firstChild.style.textDecoration='underline'" onmouseout="this.firstChild.style.textDecoration=''"><a href="admin_config.asp?user=<%=ruser%>&amp;page=1" onmouseover="return true;">[基本配置]</a></td>
				<td style="width:20%;" onclick="window.location='admin_config.asp?user=<%=ruser%>&amp;page=2'" onmouseover="this.firstChild.style.textDecoration='underline'" onmouseout="this.firstChild.style.textDecoration=''"><a href="admin_config.asp?user=<%=ruser%>&amp;page=2" onmouseover="return true;">[邮件通知]</a></td>
				<td style="width:20%;" onclick="window.location='admin_config.asp?user=<%=ruser%>&amp;page=4'" onmouseover="this.firstChild.style.textDecoration='underline'" onmouseout="this.firstChild.style.textDecoration=''"><a href="admin_config.asp?user=<%=ruser%>&amp;page=4" onmouseover="return true;">[界面尺寸]</a></td>
				<td style="width:20%;" onclick="window.location='admin_config.asp?user=<%=ruser%>&amp;page=8'" onmouseover="this.firstChild.style.textDecoration='underline'" onmouseout="this.firstChild.style.textDecoration=''"><a href="admin_config.asp?user=<%=ruser%>&amp;page=8" onmouseover="return true;">[功能设置]</a></td>
				<td style="width:20%;" onclick="window.location='admin_config.asp?user=<%=ruser%>'" onmouseover="this.firstChild.style.textDecoration='underline'" onmouseout="this.firstChild.style.textDecoration=''"><a href="admin_config.asp?user=<%=ruser%>" onmouseover="return true;">[全部参数]</a></td>
			</tr>
			</table>
			...
			%>
				...
				<%tstatus=rs("status")%>
				留言本状态：　　　　　<input type="radio" name="status1" value="1" id="status11"<%=cked(StatusOpen)%><%=dised(web_StatusOpen=false)%> /><label for="status11"<%=dised(web_StatusOpen=false)%>>开启</label>　　<input type="radio" name="status1" value="0" id="status12"<%=cked(StatusOpen=false)%><%=dised(web_StatusOpen=false)%> /><label for="status12"<%=dised(web_StatusOpen=false)%>>关闭</label> (关闭时"访客留言权限"设置无效)<br/><br/>

				访客留言权限：　　　　<input type="radio" name="status2" value="2" id="status21"<%=cked(StatusWrite)%><%=dised(web_StatusWrite=false)%> /><label for="status21"<%=dised(web_StatusWrite=false)%>>开启</label>　　<input type="radio" name="status2" value="0" id="status22"<%=cked(StatusWrite=false)%><%=dised(web_StatusWrite=false)%> /><label for="status22"<%=dised(web_StatusWrite=false)%>>关闭</label><br/><br/>

				访客搜索留言权限：　　<input type="radio" name="status3" value="4" id="status31"<%=cked(StatusSearch)%><%=dised(web_StatusSearch=false)%> /><label for="status31"<%=dised(web_StatusSearch=false)%>>开启</label>　　<input type="radio" name="status3" value="0" id="status32"<%=cked(StatusSearch=false)%><%=dised(web_StatusSearch=false)%> /><label for="status32"<%=dised(web_StatusSearch=false)%>>关闭</label><br/><br/>
				...
				管理员默认HTML权限：　<input type="checkbox" value="1" name="adminhtml" id="adminhtml"<%=cked(clng(web_adminlimit and adminlimit and 1)<>0)%><%=dised(web_AdminHTMLSupport=false)%> /><label for="adminhtml"<%=dised(web_AdminHTMLSupport=false)%>>管理员回复、公告默认支持HTML</label><br/>
				　　　　　　　　　　　<input type="checkbox" value="1" name="adminubb" id="adminubb"<%=cked(clng(web_adminlimit and adminlimit and 2)<>0)%><%=dised(web_AdminUBBSupport=false)%> /><label for="adminubb"<%=dised(web_AdminUBBSupport=false)%>>管理员回复、公告默认支持UBB</label><br/>
				　　　　　　　　　　　<input type="checkbox" value="1" name="adminertn" id="adminertn"<%=cked(clng(web_adminlimit and adminlimit and 4)<>0)%><%=dised(web_AdminAllowNewLine=false)%> /><label for="adminertn"<%=dised(web_AdminAllowNewLine=false)%>>管理员不支持HTML和UBB时，默认允许回车换行</label><br/><br/>

				<%guestlimit=rs("guesthtml")%>
				访客HTML权限：　　　　<input type="checkbox" value="1" name="guesthtml" id="guesthtml"<%=cked(clng(web_guestlimit and guestlimit and 1)<>0)%><%=dised(web_HTMLSupport=false)%> /><label for="guesthtml"<%=dised(web_HTMLSupport=false)%>>访客留言支持HTML</label><br/>
				　　　　　　　　　　　<input type="checkbox" value="1" name="guestubb" id="guestubb"<%=cked(clng(web_guestlimit and guestlimit and 2)<>0)%><%=dised(web_UBBSupport=false)%> /><label for="guestubb"<%=dised(web_UBBSupport=false)%>>访客留言支持UBB</label><br/>
				　　　　　　　　　　　<input type="checkbox" value="1" name="guestertn" id="guestertn"<%=cked(clng(web_guestlimit and guestlimit and 4)<>0)%><%=dised(web_AllowNewLine=false)%> /><label for="guestertn"<%=dised(web_AllowNewLine=false)%>>访客不支持HTML和UBB时，允许回车换行</label><br/><br/>
			
				管理员登录超时：　　　<input type="text" size="4" maxlength="4" name="admintimeout" value="<%=rs("admintimeout")%>" />分 (默认=20)<br/><br/>
				为访客显示IP前：　　　<input type="text" size="4" maxlength="1" name="showip" value="<%=ShowIP%>" />位 (可选值：0～<%=web_ShowIP%>)<br/><br/>
				为管理员显示IP前：　　<input type="text" size="4" maxlength="1" name="adminshowip" value="<%=AdminShowIP%>" />位 (可选值：0～<%=web_AdminShowIP%>)<br/><br/>
				为管理员显示原IP前：　<input type="text" size="4" maxlength="1" name="adminshoworiginalip" value="<%=AdminShowOriginalIP%>" />位 (可选值：0～<%=web_AdminShowOriginalIP%>,使用代理服务器时此项显示原始IP)<br/><br/>
				
				登录验证码长度：　　　<input type="text" size="4" maxlength="2" name="vcodecount" value="<%=VcodeCount%>" />位 (可选值：<%=web_VcodeCount%>～10)<br/><br/>
				留言验证码长度：　　　<input type="text" size="4" maxlength="2" name="writevcodecount" value="<%=WriteVcodeCount%>" />位 (可选值：<%=web_WriteVcodeCount%>～10)<br/><br/><br/>
			<%
			end if
			...
			%>
				...
				<span style="font-weight:bold">邮件通知（请勿使用重要邮箱以防泄密）：</span><br/><br/>
				新留言到达通知：　　　<input type="checkbox" value="1" name="mailnewinform" id="mailnewinform"<%=cked(clng(web_MailFlag and MailFlag and 1)<>0)%><%=dised(web_MailNewInform=false)%> /><label for="mailnewinform"<%=dised(web_MailNewInform=false)%>>启用</label><br/><br/>
				版主回复通知选项：　　<input type="checkbox" value="1" name="mailreplyinform" id="mailreplyinform"<%=cked(clng(web_MailFlag and MailFlag and 2)<>0)%><%=dised(web_MailReplyInform=false)%> /><label for="mailreplyinform"<%=dised(web_MailReplyInform=false)%>>开启</label><br/><br/>
			...
			%>
				...
				“验证码”文本框宽度：<input type="text" size="10" maxlength="2" name="leavevcodewidth" value="<%=rs("leavevcodewidth")%>" /> (默认=10,单位:字母宽度)<br/><br/>
			...
			%>	
				...
				UBB开关(须启用UBB)：　<input type="checkbox" name="ubbflag_image" id="ubbflag_image" value="1"<%=cked(web_UbbFlag_image and UbbFlag_image)%><%=dised(web_UbbFlag_image=false)%> /><label for="ubbflag_image"<%=dised(web_UbbFlag_image=false)%>>图片</label>　　　
									...
									<input type="checkbox" name="ubbflag_autourl" id="ubbflag_autourl" value="1"<%=cked(web_UbbFlag_autourl and UbbFlag_autourl)%><%=dised(web_UbbFlag_autourl=false)%> /><label for="ubbflag_autourl"<%=dised(web_UbbFlag_autourl=false)%>>自动识别网址</label><br/>
				...
				　　　　　　　　　　　<input type="checkbox" name="ubbflag_player" id="ubbflag_player" value="1"<%=cked(web_UbbFlag_player and UbbFlag_player)%><%=dised(web_UbbFlag_player=false)%> /><label for="ubbflag_player"<%=dised(web_UbbFlag_player=false)%>>播放控件</label>　
									...
									<input type="checkbox" name="ubbflag_fontstyle" id="ubbflag_fontstyle" value="1"<%=cked(web_UbbFlag_fontstyle and UbbFlag_fontstyle)%><%=dised(web_UbbFlag_fontstyle=false)%> /><label for="ubbflag_fontstyle"<%=dised(web_UbbFlag_fontstyle=false)%>>字体样式</label><br/>
				...
				　　　　　　　　　　　<input type="checkbox" name="ubbflag_fontcolor" id="ubbflag_fontcolor" value="1"<%=cked(web_UbbFlag_fontcolor and UbbFlag_fontcolor)%><%=dised(web_UbbFlag_fontcolor=false)%> /><label for="ubbflag_fontcolor"<%=dised(web_UbbFlag_fontcolor=false)%>>字体颜色</label>　
									...
									<input type="checkbox" name="ubbflag_movement" id="ubbflag_movement" value="1"<%=cked(web_UbbFlag_movement and UbbFlag_movement)%><%=dised(web_UbbFlag_movement=false)%> /><label for="ubbflag_movement"<%=dised(web_UbbFlag_movement=false)%>>移动效果(<span title="启用移动效果将无法通过W3C XHTML 1.0 Traditional 认证">反对使用</span>)</label><br/>

				　　　　　　　　　　　<input type="checkbox" name="ubbflag_cssfilter" id="ubbflag_cssfilter" value="1"<%=cked(web_UbbFlag_cssfilter and UbbFlag_cssfilter)%><%=dised(web_UbbFlag_cssfilter=false)%> /><label for="ubbflag_cssfilter"<%=dised(web_UbbFlag_cssfilter=false)%>>滤镜效果</label>　
									<input type="checkbox" name="ubbflag_face" id="ubbflag_face" value="1"<%=cked(web_UbbFlag_face and UbbFlag_face)%><%=dised(web_UbbFlag_face=false)%> /><label for="ubbflag_face"<%=dised(web_UbbFlag_face=false)%>>表情图标</label><br/><br/>
...
function check()
{
	...
	if ((showpage & 1) != 0)
	{
		...
		else if (tv<0 || tv><%=web_ShowIP%> || document.configform.showip.value=='')
			{alert('“为访客显示IP”必须在0～<%=web_ShowIP%>的范围内。');document.configform.showip.select();return false;}
		...
		else if (tv<0 || tv><%=web_AdminShowIP%> || document.configform.adminshowip.value=='')
			{alert('“为管理员显示IP”必须在0～<%=web_AdminShowIP%>的范围内。');document.configform.adminshowip.select();return false;}
		...
		else if (tv<0 || tv><%=web_AdminShowOriginalIP%> || document.configform.adminshoworiginalip.value=='')
			{alert('“为管理员显示原IP”必须在0～<%=web_AdminShowOriginalIP%>的范围内。');document.configform.adminshoworiginalip.select();return false;}
		...
		if (isNaN(tv=Number(document.configform.vcodecount.value)))
			...
			{alert('“登录验证码长度”必须在<%=web_VcodeCount%>～10的范围内。');document.configform.vcodecount.select();return false;}
		...
		if (isNaN(tv=Number(document.configform.writevcodecount.value)))
			...
			{alert('“留言验证码长度”必须在<%=web_WriteVcodeCount%>～10的范围内。');document.configform.writevcodecount.select();return false;}
	}
	if ((showpage & 2) != 0)
	{
