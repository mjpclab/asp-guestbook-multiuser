...
sub show_page_list(byval CurPage,byval PagesCount,byval filename,byval pagetitle,byval param)
	...
	if ShowAdvPageList then
		...
		str_first_page=		"<a class=""page-control"" name=""page-control"" href=""" &filename& "?page=" &first_page_no& param & "&user=" &ruser& """><img src=""asset/image/icon_page_first.gif"" title=""第" &first_page_no& "页"" /></a>"
		str_largeprev_page=	"<a class=""page-control"" name=""page-control"" href=""" &filename& "?page=" &largeprev_page_no& param & "&user=" &ruser& """><img src=""asset/image/icon_page_largeprev.gif"" title=""第" &largeprev_page_no& "页"" /></a>"
		str_prev_page=		"<a class=""page-control"" name=""page-control"" href=""" &filename& "?page=" &prev_page_no& param & "&user=" &ruser& """><img src=""asset/image/icon_page_prev.gif"" title=""第" &prev_page_no& "页"" /></a>"

		str_last_page=		"<a class=""page-control"" name=""page-control"" href=""" &filename& "?page=" &last_page_no& param & "&user=" &ruser& """><img src=""asset/image/icon_page_last.gif"" title=""第" &last_page_no& "页"" /></a>"
		str_largenext_page=	"<a class=""page-control"" name=""page-control"" href=""" &filename& "?page=" &largenext_page_no& param & "&user=" &ruser& """><img src=""asset/image/icon_page_largenext.gif"" title=""第" &largenext_page_no& "页"" /></a>"
		str_next_page=		"<a class=""page-control"" name=""page-control"" href=""" &filename& "?page=" &next_page_no& param & "&user=" &ruser& """><img src=""asset/image/icon_page_next.gif"" title=""第" &next_page_no& "页"" /></a>"
...
sub showadminicons()%>
	...
	<%if admin_msnid<>"" then%><span class="icon" title="版主Skype：<%=admin_msnid%>"><img src="asset/image/icon_skype.gif"/></span><%end if%>
...
<div class="admin-message-tools">
	...
	<div class="group">
		...
		<div class="tools">
			...
			<%if clng(guestflag and 128)<>0 then%><span class="tool"><img src="asset/image/icon_mail.gif" />回复通知<%if (web_MailReplyInform and MailReplyInform)=false then response.write ",已禁用"%></span><%end if%>
		</div>
	</div>
	<%end if%>
</div><%
...
<div class="admin-message-tools">
	...
	<%if clng(guestflag and 16)<>0 then%><span class="tool"><img src="asset/image/icon_wait2pass.gif">待审核</span><%end if%>
	<%if clng(guestflag and 32)<>0 then%><span class="tool"><img src="asset/image/icon_whisper.gif">悄悄话<%if clng(guestflag and 64)<>0 then response.write ",已加密"%></span><%end if%>
	<%if rs.Fields("parent_id")<0 then%><span class="tool"><img src="asset/image/icon_toplocked.gif" />留言已置顶</span><%end if%>
	<%if clng(guestflag and 512)<>0 and rs.Fields("parent_id")<=0 then%><span class="tool"><img src="asset/image/icon_lockreply.gif" />回复已锁定</span><%end if%>
	<%if clng(guestflag and 256)<>0 then%><span class="tool"><img src="asset/image/icon_hidecontact.gif" />联系已隐藏</span><%end if%>
	<%if clng(guestflag and 40)=8 then%><span class="tool"><img src="asset/image/icon_hide.gif" />内容已隐藏</span><%end if%>
	<%if clng(guestflag and 128)<>0 and MailReplyInform=true then%><span class="tool"><img src="asset/image/icon_mail.gif" />回复通知</span><%end if%>
</div><%
end sub
...
<div class="message inner-message admin-message">
	...
	<div class="words">
		...
		<%else
			...
			if left(pagename,9)="admin_web" and AdminViewCode=true then
...
<div class="message outer-message guest-message">
	<div class="info">
		...
		<%if (iswhisper=false and clng(guestflag and 256)=0) or (pagename="showword" and needverify) or left(pagename,5)="admin" then%>
			<div class="icons"><%showguestinfoicons(rs)%></div>
		<%end if%>
	...
	<div class="detail">
		...
		<%if rs.Fields("parent_id")<=0 and left(pagename,5)<>"admin" and left(pagename,3)<>"web" then showguestreplyicons rs.Fields("id"),rs.Fields("parent_id"),StatusWrite and StatusGuestReply and clng(rs.Fields("guestflag") and 512)=0%>
		<div class="words">
			<%if iswhisper=true and pagename<>"showword" and left(pagename,5)<>"admin" then%>
				<span class="inner-hint"><img src="asset/image/icon_whisper.gif"/>(给版主的悄悄话...)</span>
			<%elseif ishidden=true and left(pagename,5)<>"admin" then%>
		...
		elseif (pagename="web_admin_search" or pagename="web_showword") and ReplyInWord=true then
