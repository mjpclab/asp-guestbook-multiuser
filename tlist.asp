...
if mode="json" then
	...
	do while not rs.EOF
		if Not CBool(rs.Fields("guestflag") and 48) then
			...
			Response.write "{""title"":""" & jsfilter(str_title) & """,""fulltitle"":""" & jsfilter(rs.Fields("title")) & """,""url"":""" & geturlpath & "index.asp?user=" &ruser& "&page=" & page & "#a" & (((i-1) mod CountPerPage)+1) & """}"
		end if
...
elseif mode="js" then
	...
	%>document.write('<ul><%do while not rs.EOF
		...
		if Not CBool(rs.Fields("guestflag") and 48) then
			output_counter=output_counter+1
			if max_len=0 then str_title=rs.Fields("title") else str_title=left(rs.Fields("title"),max_len) end if
			%><li><a href="<%=geturlpath & "index.asp?user=" &ruser& "&page=" & page & "#a" & (((i-1) mod CountPerPage)+1)%>" target="<%=t%>" title="<%=jsfilter(rs.Fields("title"))%>"><%=jsfilter(pre & str_title)%></a></li><%
		end if
		if i mod CountPerPage=0 then page=page+1
		if output_counter=n then exit do
		rs.MoveNext
...
<body class="tlist"<%=bodylimit%>>
	...
	<%do while not rs.EOF
		...
		if Not CBool(rs.Fields("guestflag") and 48) then
			output_counter=output_counter+1
			if max_len=0 then str_title=rs.Fields("title") else str_title=left(rs.Fields("title"),max_len) end if%><li><a href="<%=geturlpath & "index.asp?user=" &ruser& "&page=" & page & "#a" & (((i-1) mod CountPerPage)+1)%>" target="<%=t%>" title="<%=rs.Fields("title")%>"><%=pre & str_title%></a></li>
		<%end if
		if i mod CountPerPage=0 then page=page+1
		if output_counter=n then exit do
		rs.MoveNext
