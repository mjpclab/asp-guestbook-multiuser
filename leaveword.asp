		...
		<td style="width:100%; background-color:<%=TableContentBGC%>;">
			<form method="post" action="write.asp" onsubmit="return submitcheck(this)" name="form1">
				...
				<div id="divRequired" style="padding:5px;">
					...
					<%if WriteVcodeCount>0 then%>验证码<span style="color:#FF0000">*</span><input type="text" name="ivcode" size="<%=LeaveVcodeWidth%>" autocomplete="off"/> <img src="show_vcode.asp?user=<%=request("user")%>" onclick="this.src=this.src" /><br/><br/><%end if%>
					...
					标　题<span style="color:#FF0000">*</span><input type="text" name="ititle" size="<%=LeaveTextWidth%>" maxlength="30" value="<%=server.htmlEncode(FormOrSession(InstanceName & "_ititle_" & Request("user")))%><%if Request.Form("ititle")="" and isnumeric(Request("follow")) and Request("follow")<>"" then response.write "Re:"%>"/><br/><br/>
					
					内容： <%=getstatus(web_HTMLSupport and HTMLSupport)%>HTML标记　<%=getstatus(web_UBBSupport and UBBSupport)%>UBB标记<%if not(web_HTMLSupport and HTMLSupport) and not(web_UBBSupport and UBBSupport) and (web_AllowNewLine and AllowNewLine) then Response.Write "　" & getstatus(true) & "允许换行"%>
					
					<br/><textarea name="icontent" id="icontent" cols="<%=LeaveContentWidth%>" rows="<%=LeaveContentHeight%>" onkeydown="icontent_keydown(arguments[0]);"<%if WordsLimit>0 then Response.Write " onpropertychange=""checklength(this,"&WordsLimit&");"""%>><%=server.htmlEncode(FormOrSession(InstanceName & "_icontent_" & Request("user")))%></textarea>
