...
function deleteSaved(requestField,sql)
	...
	if listids<>"" then
		...
		cn.Execute Replace(Replace(sql,"{0}",listids),"{1}",adminid),,129
	end if
end function
...
function addNewIPv4(requestField,sql)
	...
	for each iprange in entries
		...
		if isIPv4Range(ipfrom) and isIPv4Range(ipto) then
			cn.Execute Replace(Replace(Replace(sql,"{0}",ipv4ToHex(ipfrom,false)),"{1}",ipv4ToHex(ipto,true)),"{2}",adminid),,129
		end if
	next
end function
...
function addNewIPv6(requestField,sql)
	...
	for each iprange in entries
		...
		if isIPv6Range(ipfrom) and isIPv6Range(ipto) then
			cn.Execute Replace(Replace(Replace(sql,"{0}",ipv6ToHex(ipfrom,false)),"{1}",ipv6ToHex(ipto,true)),"{2}",adminid),,129
		end if
	next
end function
...
if Not IsEmpty(Request.Form) then
	...
	cn.Execute Replace(Replace(sql_adminsaveipconfig_update,"{0}",tipconstatus),"{1}",adminid),,129
